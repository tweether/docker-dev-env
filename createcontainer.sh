...
function createcontainer(){
    ...
    #此处创建系统容器，用于开发者开发使用
    docker create --name "$1" --volumes-from "$1"DV -p "$2":22 -p "$3":8888 --security-opt seccomp=unconfined --privileged=true base_os:v1 /usr/sbin/init
    docker start $1
    docker cp ./createuser.sh $1:/root
...
if [ $# == 3 ]
then
    createcontainer $stringname $stringport $stringdir
fi
